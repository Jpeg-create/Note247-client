...
export default function Dashboard() {
  ...
  const deleteDoc = async (shortId) => {
    ...
    try {
      ...
      setDocs((prev) => prev.filter((doc) => doc.short_id !== shortId));
    } catch (err) {
  ...
  const copyLink = async (shortId) => {
    ...
    try {
      ...
      setError('Could not copy link in this browser tab.');
    }
  };
            ...
            {docs.map((doc) => (
              ...
              >
                <div className={styles.docHeader}>
                  ...
                  <span
                    className={styles.docLang}
                    style={{ color: langColors[doc.language] || '#7070a0' }}
                  >
                    {doc.language}
                  </span>
                </div>
                ...
                <div className={styles.docActions} onClick={(e) => e.stopPropagation()}>
                  <button className="btn sm ghost" onClick={() => copyLink(doc.short_id)}>
                    Copy Link
                  </button>
                  <button className="btn sm ghost" onClick={() => deleteDoc(doc.short_id)}>
                    Delete
                  </button>
                </div>
              </div>
